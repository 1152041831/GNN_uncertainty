...
class ModelLightningModule(MatglLightningModuleMixin, pl.LightningModule):
    ...
    def step(self, batch: tuple):
        ...
        if isinstance(self.model, M3GNet):
            ...
            # print(f"当前预测值形状{preds.shape}: ", preds.dim())
            if preds.dim() == 1:
                # 如果形状为[2]，则在第0维上增加一维, shape: [2] -> [1,2]
                preds = torch.unsqueeze(preds, dim=0)
            # print(f"当前预测值形状{preds.shape}: ", preds.dim())
            # print(f"真值{labels.shape}: ", labels)

            delta = torch.abs(labels - preds[:, 1]) # |Ptrue - P|
            # print(f"不确定度u{delta.shape}: ", delta)

            labels = torch.stack((delta, labels), dim=1)
            # print(f"为真值添加不确定度后{labels.shape}: ", labels)
    ...
    def loss_fn(self, loss: nn.Module, labels: torch.Tensor, preds: torch.Tensor):
        ...
        # loss =(Ptrue -P)² + (|Ptrue - P| - |u|)²
        # print("loss function: ", loss)
        # a = torch.tensor([1,2,2])
        # b = torch.tensor([0,0,0])
        # a = a.float()
        # b = b.float()
        # print("test loss: ", loss(a,b))


        # loss 默认为mse
        diff = torch.abs(labels[:, 1] - preds[:, 1])  # |Ptrue - P|
        u_loss = loss(diff, preds[:, 0]) # (|Ptrue - P| - u)²
        total_loss = loss(labels[:, 1], preds[:, 1]) + u_loss  # 计算总损失 (Ptrue -P)² + (|Ptrue - P| - u)²
        # 预测值 + 不确定度 的mae ,rmse
        mae = self.mae(labels[:, 1], preds[:, 1]) + self.mae(labels[:, 0], preds[:, 0])
        rmse = self.rmse(labels[:, 1], preds[:, 1]) + self.rmse(labels[:, 0], preds[:, 0])
